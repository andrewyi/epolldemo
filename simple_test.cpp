...
int main() {
    ...
    int current_fd_count = 1; // only listen fd is waited at this time
    ...
    while (TRUE) {
        sleep(3); // test option: to avoid epoll_wait wake up too frequently
        std::cout << "loop, gonna epoll_wait" << std::endl;
        ready_count = epoll_wait(epoll_fd, events, current_fd_count, 3000/*timeout*/); // 100 means 100 milliseconds
        if (ready_count == -1) {
            if (errno != EINTR) {
        ...
        for (int j=0; j!=ready_count; j++) {
            ...
            if (event.data.fd == listen_fd) {
                ...
                while (TRUE) {
                    ...
                    /* test case: we close read first, to see EPOLLHUP events events
                    if (shutdown(fd, SHUT_RD) == -1) { // shutdown read directly!
                        ERROR_EXIT("fail to shutdown read of accepted fd");
                    }
                    */

                    /* test case: we close write first, to see EPOLLHUP or events
                    if (shutdown(fd, SHUT_WR) == -1) { // shutdown write directly!
                        ERROR_EXIT("fail to shutdown write of accepted fd");
                    }
                    */

                    /* test case: we wait while crash the remote(without receiving fin) and write some data to remote, to see
                     *  1. the connection timeout error
                     *  2. the connection reset error
                    sleep(10); // enought to put the network down
                    if (write(fd, (void *)tmp_content, 1) == -1) {
                        ERROR_EXIT("fail to write to fd");
                    }
                    */

                    struct epoll_event iev;
                    iev.events = EPOLLIN | EPOLLOUT ; // | EPOLLET ; // test option, edge trigger or not
                    iev.data.fd = fd;
                ...
                if ( (event.events & (EPOLLERR|EPOLLHUP)) != 0 ) {
                    ...
                    if ( (event.events & EPOLLHUP) != 0 ) {
                        std::cerr << "EPOLLHUP fd hang up" << std::endl;
                        error_code_len = sizeof(error_code);
                        if ( getsockopt(fd, SOL_SOCKET, SO_ERROR, &error_code, (socklen_t *)&error_code_len) == -1 ) {
                            ERROR_NO_EXIT("fail to get fd error flag with EPOLLHUP");
                        } else {
                            std::cerr << "fd epollhup: " << strerror(error_code) << ", fd: " << fd << std::endl;
                        }
                    }
                    ...
                    if ( (event.events & EPOLLERR) != 0 ) {
                        ...
                        error_code_len = sizeof(error_code);
                        if ( getsockopt(fd, SOL_SOCKET, SO_ERROR, &error_code, (socklen_t *)&error_code_len) == -1 ) {
                            ERROR_EXIT("fail to get fd error flag with EPOLLERR");
                        }
                        // print error message and exit 
                        std::cerr << "fd epollerr: " << strerror(error_code) << ", fd: " << fd << std::endl;
                    } 

                    // remove the fd because of error
                    to_remove_fd_sets.insert(fd);
                } 

                if ( (event.events & EPOLLOUT) != 0 ) { // read into content
                    std::cout << "could write data to fd: " << fd << std::endl;
                    std::cout << "EPOLLOUT" << std::endl;

                    /* test case: try close directly
                    to_remove_fd_sets.insert(fd);
                    */

                    /* test case: write something
                    if (write(fd, (void *)tmp_content, 1) == -1) {
                        ERROR_EXIT("fail to write to fd");
                    }
                    */
                ...
                /* test case: false to disable read and keep data in buffers */
                    ...
                    while (TRUE) {
                        ...
                        if (byte_count == -1) {
                            if (errno == EAGAIN || errno == EWOULDBLOCK) {
                                std::cout << "all received data has been fetched!" << std::endl;
                                // sleep(10); // test case: we continue to send data here, will events be triggered?
                                break;
                            } else if (errno == EINTR) {
                                continue;
                            } else {
                                ERROR_NO_EXIT("fail to read from fd"); // absolutely this should not happen in normal case
                                to_remove_fd_sets.insert(fd);
                                break;
                            }

                        } else if (byte_count > 0) {
                            // pass, we do not want to show the results
                            std::cout << "read from byte count: " << byte_count << " from fd: " << fd << std::endl;

                        } else if (byte_count == 0){
                            std::cout << "seems remote close write, fd: " << fd << std::endl;
                            /* test case: we close write later, to see whether EPOLLHUP will happen when remote closes
                            if (shutdown(fd, SHUT_WR) == -1) { // shutdown write directly!
                                ERROR_EXIT("fail to shutdown write of accepted fd");
                            }
                            */
                            /* test case: do not close it to test multiple events */
                            to_remove_fd_sets.insert(fd);
                            break;
        ...
        for (std::set<int>::iterator it = to_remove_fd_sets.begin(); it != to_remove_fd_sets.end(); it ++) { // actual delete
            /* std::cout << "remove fd: " << *it << std::endl; */ // remove silently
